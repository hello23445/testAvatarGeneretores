...
// ============ ЖЕСТКАЯ ПРОВЕРКА ПОЛНОЭКРАННОГО РЕЖИМА ============
function forceFullscreenCheck() {
    try {
        const fullscreenMode = localStorage.getItem('fullscreenMode');
        const elem = document.documentElement;
        
        // Проверяем текущее состояние полноэкрана
        ...
        console.log(`[FULLSCREEN] Режим: ${fullscreenMode}, Текущее состояние: ${isCurrentlyFullscreen ? 'ON' : 'OFF'}`);
        
        if (fullscreenMode === 'enabled') {
            // ВКЛЮЧАЕМ полноэкранный режим
            if (!isCurrentlyFullscreen && elem) {
                console.log('[FULLSCREEN] Попытка включить полноэкран...');
                
                // Пытаемся включить через все доступные методы
                if (elem.requestFullscreen && typeof elem.requestFullscreen === 'function') {
                    elem.requestFullscreen().catch(err => {
                        console.warn('[FULLSCREEN] requestFullscreen ошибка:', err.message);
                    });
                } else if (elem.webkitRequestFullscreen && typeof elem.webkitRequestFullscreen === 'function') {
                    console.log('[FULLSCREEN] Используем webkit');
                    elem.webkitRequestFullscreen();
                } else if (elem.mozRequestFullScreen && typeof elem.mozRequestFullScreen === 'function') {
                    console.log('[FULLSCREEN] Используем mozilla');
                    elem.mozRequestFullScreen();
                } else if (elem.msRequestFullscreen && typeof elem.msRequestFullscreen === 'function') {
                    console.log('[FULLSCREEN] Используем ms');
                    elem.msRequestFullscreen();
                } else {
                    console.warn('[FULLSCREEN] Полноэкран недоступен в этом браузере');
                }
            }
        } 
        else {
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        console.log('[FULLSCREEN] DOMContentLoaded - первая проверка');
        forceFullscreenCheck();
    });
} else {
    console.log('[FULLSCREEN] Документ уже загружен - первая проверка');
    forceFullscreenCheck();
}

// Жесткая проверка каждые 500ms
setInterval(forceFullscreenCheck, 500);

// Дополнительная проверка при изменении localStorage
window.addEventListener('storage', (event) => {
    if (event.key === 'fullscreenMode') {
        console.log('[FULLSCREEN] localStorage изменен, выполняем проверку');
        forceFullscreenCheck();
    }
});

// Проверка при получении фокуса окна
window.addEventListener('focus', () => {
    console.log('[FULLSCREEN] Окно получило фокус, проверяем режим');
    forceFullscreenCheck();
});

// Логируем все попытки изменения полноэкранного режима браузером
document.addEventListener('fullscreenchange', () => {
    console.log('[FULLSCREEN] Событие fullscreenchange');
    forceFullscreenCheck();
});

document.addEventListener('webkitfullscreenchange', () => {
    console.log('[FULLSCREEN] Событие webkitfullscreenchange');
    forceFullscreenCheck();
});

document.addEventListener('mozfullscreenchange', () => {
    console.log('[FULLSCREEN] Событие mozfullscreenchange');
    forceFullscreenCheck();
});

document.addEventListener('MSFullscreenChange', () => {
    console.log('[FULLSCREEN] Событие MSFullscreenChange');
    forceFullscreenCheck();
});
// ============ КОНЕЦ ЖЕСТКОЙ ПРОВЕРКИ ============
